...
struct ContentView: View {
    ...
    @State private var editingEntry: CrossingEntry?
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("Tap + to log your first crossing.")
                .font(Theme.captionFont)
                .foregroundStyle(.secondary)
        }
    ...
    private func row(for entry: CrossingEntry) -> some View {
        HStack {
...
struct EntryEditView: View {
    ...
    let existingId: UUID?
    let onSave: (CrossingEntry) -> Void

    init(entry: CrossingEntry?, onSave: @escaping (CrossingEntry) -> Void) {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Crossing details") {
                    TextField("Title", text: $title)
                        .accessibilityIdentifier("titleField")
                    TextField("Crossing", text: $vendor)
            ...
            .navigationTitle(existingId == nil ? "Add Crossing" : "Edit Crossing")
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        let entry = CrossingEntry(
                            id: existingId ?? UUID(),
                            title: title.isEmpty ? "Crossing" : title,
                            vendor: vendor,
                            amount: amount,
                            date: date,
                            notes: notes
