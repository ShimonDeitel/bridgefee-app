...
final class BridgefeeUITests: XCTestCase {
    ...
    func testFreeLimitTriggersPaywall() throws {
        ...
        for i in 0..<12 {
            ...
            if titleField.waitForExistence(timeout: 3) {
                ...
                app.buttons["saveEntryButton"].tap()
            } else if app.staticTexts["Bridgefee Pro"].waitForExistence(timeout: 3) {
                break
            }
        }
        XCTAssertTrue(app.staticTexts["Bridgefee Pro"].waitForExistence(timeout: 5))
    }
